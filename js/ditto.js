...
/*
 需要注意的是:
 location的hash部分
 window.onhashchange实现ajax请求内容时  使用浏览器后退和前进功能
 hash就是url中#及后面的部分例如:www.google.com.hk#123的#123  当只有hash部分发生变化时  浏览器的历史记录会产生记录  但不会影响向服务器发出请求
 这时按后退键地址栏的url会变化但页面内容也随之变化  但是并不发出请求
 * */
...
function initialize() {
  ...
  // page router
  router();
  //此处是当window.location.hash值发生改变的时候  就触发该事件
